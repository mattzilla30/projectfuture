...
class CookieJar(context: Context) {
    ...
    fun cookieHeaderFor(url: Url): String? {
        ...
        val matching = cookies.filter {
            domainMatches(it.domain, url.host) && url.path.startsWith(it.path) && (!it.secure || url.isHttps)
        }
    ...
    private fun save() {
        val raw = cookies.joinToString("\n") {
            "${it.name}\t${it.value}\t${it.domain}\t${it.path}\t${it.expiresAt ?: "-"}\t${if (it.secure) "1" else "0"}"
        }
...
fun parseSetCookie(header: String, requestHost: String): Cookie? {
    ...
    for (attr in parts.drop(1)) {
        ...
        when (kv[0].trim().lowercase()) {
            "domain" -> kv.getOrNull(1)?.trim()?.removePrefix(".")?.let { if (it.isNotEmpty()) domain = it }
    ...
    return Cookie(nameValue[0].trim(), nameValue[1].trim(), domain, path, expiresAt, secure)
}
